...
def harrison_ford
  ...
  Movie.joins(:actors).where.not(castings: {ord: 1}).select(:id, :title).where(actors: {name: 'Harrison Ford'})
end
...
def biggest_cast
  ...
  Movie.joins(:castings).group(:id).order('COUNT(actor_id) DESC').limit(3).select(:id, :title)
...
def directed_by_one_of(them)
  ...
  Movie.joins(:director).where('name IN (?)', them).select(:id, :title)
